...
cdo -r selvar,rsns,rsnl,rsnscl,rtnlcl,rsnlcl,rts,rsnt,clt,rtl,cl,clw $radfile $NewTag.RAD.mon.mean.${YEAR1}-${YEAR2}.nc 
...
#=================================================== done
        ...
        cp -v $NewTag.???.all.mon.mean.${YEAR1}-${YEAR2}.nc pprcmdata/monthly/

        # selected variables monthly mean data
        cp -v $NewTag.???.mon.mean.${YEAR1}-${YEAR2}.nc pprcmdata/monthly/
        cp -v $NewTag.???.ymon.mean.${YEAR1}-${YEAR2}.nc pprcmdata/monthly/

        # selected variables seasonal mean data
        cp -v $NewTag.???.ymon.mean.${YEAR1}-${YEAR2}.MJJASO.nc pprcmdata/monthly/
        cp -v $NewTag.???.ymon.mean.${YEAR1}-${YEAR2}.NDJFMA.nc pprcmdata/monthly/
...
echo ------------------- to save RAD ATM in 1996 2005 ------------
for tag in RAD ATM
do
    for year in 1996
    do
        cp -v  SWIO_$tag.$year????00.nc pprcmdata/daily
    done
done
...
do
    for year in $( seq -s " " ${YEAR1} ${YEAR2} )
    do
        cp -v SWIO?$tag?$year*.nc pprcmdata/daily
    done
done
...
do
    cp -v $NewTag.$tag.all.daymean.${YEAR1}-${YEAR2}.nc $Dir/pprcmdata/daily/
done
...
cp -v *.all.day.fldmean.${YEAR1}-${YEAR2}.nc pprcmdata/fldmean
cp -v *.all.mon.fldmean.${YEAR1}-${YEAR2}.nc pprcmdata/fldmean
...
cp -v $NewTag.???.daystd.${YEAR1}-${YEAR2}.nc pprcmdata/statistics
cp -v $NewTag.???.dayvar.${YEAR1}-${YEAR2}.nc pprcmdata/statistics
